...
(function($) {
	$(document).ready(function() {
		...
		$(document.body).on('click.vagabondOpenMediaManager', '.custom-media-button', function(e) {
			...
			media_frame = wp.media.frames.media_frame = wp.media({
				...
				multiple: false,
				// Set custom media workflow title using the localized script object 'vagabondImageUpload'.
				title: vagabondImageUpload.frame_title,
				// Limit media library access to images only.
				library: {
					type: 'image'
				},
				// Set custom button text using the localized script object 'vagabondImageUpload'.
				button: {
					text: vagabondImageUpload.frame_button
				}
			});

			media_frame.on('select', function() {
				// Grab our attachment selection and construct a JSON representation of the model.
				var media_attachment = media_frame
					.state()
					.get('selection')
					.first()
					.toJSON();
